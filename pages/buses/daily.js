...
import { checkinBuses, noBooking, getDeparture, getDestination, bookingDaily } from "../../actions/dailyAction";
import { getDaily } from '../../actions/dailyAction'
...
function Daily(props) {
    ...
    let success = props.daily.checkin?.success
                ...
                <section className="relative py-20">
                    ...
                    <div className="container mx-auto px-4">
                        <div className="items-center flex flex-wrap">
                            {bookingForm && <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
                                ...
                                <form onSubmit={booking}>
                                    ...
                                    {error ? <div className="text-red-500 text-center mb-3 font-bold">
                                        <small> {error?.data?.fullname}</small>
                                    </div> : ''}
                                    <div className="relative w-full mb-3">
                                        <label
                                            className="block uppercase text-blueGray-600 text-xs font-bold mb-2"
                                        ...
                                        />
                                        {error ? <div className="text-red-500 text-center mb-3 font-bold">
                                            <small> {error?.data?.phonenumber}</small>
                                        </div> : ''}
                                    </div>
                                    <div className="relative w-full mb-3">
                                        <label
                            ...
                            {props.daily.checkin && props.daily.checkin.data && props.daily.checkin?.data.map((res, i) => {
                                return res.seats - res.takenseat.length > 0 ? (
                                    <div key={i} className="w-full md:w-4/12 px-4 text-center cursor-pointer" onClick={() => selectThisbus(res)} >
                                        <div className={res._id === datafromselect?.id ? "relative flex flex-col min-w-0 break-words text-white bg-teal-500 w-full mb-8 shadow-lg rounded-lg" : "relative flex flex-col min-w-0 break-words bg-white w-full mb-8 shadow-lg rounded-lg"}>
                                            <div className="px-4 py-5 flex-auto">
                                                ...
                                                <h1 className="text-md font-semibold"> for {no_of_seat} seats {res.price * no_of_seat} birr</h1>
                                                <h6 className="text-md "> service charge 20birr/person</h6>
                                                <h6 className="text-md "> Total service charge {no_of_seat * serviceCharge} birr</h6>
                                                <h6 className="text-md ">Total : {no_of_seat * serviceCharge + res.price * no_of_seat} birr</h6>
                                                <h6 className="text-md ">Bus : {res.bus}</h6>
                                                <h6 className="text-md ">Available seats {res.seats - res.takenseat.length}</h6>
                                                <p className="mt-2 mb-4">Number of seats are {res.seats}
                                                </p>
                                    ...
                                    <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-8 shadow-lg rounded-lg">
                                        <div className="px-4 py-5 flex-auto">
                                            ...
                                            <h6 className="text-xl font-semibold">we have Finished ticket</h6>
                                            <p className="mt-2 mb-4 text-blueGray-500">
                                                Please try check other day.
                            ...
                            <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-200 border-0">
                                <div className="rounded-t mb-0 px-6 py-6">
                                    ...
                                    <div className="flex-auto px-4 lg:px-10 py-10 pt-0">
                                        ...
                                        {props.daily.checkin?.error ? <div className="text-red-500 text-center mb-3 font-bold">
                                            <small> {props.daily.checkin?.error}</small>
                                        </div> : ''}
                                        ...
                                        <form onSubmit={checkin}>
                                            ...
                                            <div className="relative w-full mb-3">
                                                ...
                                                <select onChange={e => setDeparture(e.target.value)} className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                                                    ...
                                                    {props.daily.departure && props.daily.departure.success ?
                                                        props.daily.departure?.success.map((item) => (
                                            ...
                                            <div className="relative w-full mb-3">
                                                ...
                                                <select onChange={e => setDestination(e.target.value)} className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                                                    <option value=''>select</option>
                                                    {props.daily.destination && props.daily.destination.success ?
                                                        props.daily.destination?.success.map((item) => (
                                            ...
                                            <div className="relative w-full mb-3">
                                                ...
                                                <input
                                                    ...
                                                    value={no_of_seat} onChange={e => setNo_of_seat(e.target.value)}
                                                />
                                                {data ? <div className="text-red-500 text-center mb-3 font-bold">
                                                    <small> {data.seats}</small>
                                                </div> : ''}
...
export default connect(mapStateToProps, { checkinUser, bookingDaily, getDaily, getDeparture, getDestination, checkinBuses, noBooking })(Daily)
